...
- Pressing Escape while an expanded drawing board has an active local stroke cancels only that draft stroke: the active stroke canvas is cleared, pointer capture is released, propagation is stopped before the app-level Escape close handler, and no stroke is submitted to the server on the subsequent pointerup. Releasing a freeform stroke outside the rendered canvas area also cancels the draft instead of submitting it; the hit test uses the visible `object-fit: contain` canvas bounds, not the fullscreen element's letterboxed area.
