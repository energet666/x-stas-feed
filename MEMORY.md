...
- Chrome side/debug panel blur depends on the final production CSS containing unprefixed `backdrop-filter`, not only `-webkit-backdrop-filter`. `@tailwindcss/vite` production Lightning CSS optimization collapsed backdrop filter declarations to the prefixed property, so it is disabled in `vite.config.ts`; Vite's normal CSS minifier keeps both declarations. Keep the `.glass-panel.side-glass-panel` override so the side panel blur/background wins over the base `.glass-panel` rule.
