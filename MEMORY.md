...
- The chosen initial poster timestamp is cached in `.posters` as a small `.poster-time` file keyed by media ID, source file size, source modtime, and smart-selection algorithm version. This keeps repeated `time=0` requests on the JPEG cache path without rerunning FFmpeg analysis; stale or invalid timestamp cache files are ignored and recomputed.
...
- Server-enabled library scans also convert discovered root image files into image-backed boards using the same storage model as uploads: the image bytes move under `.boards/{boardID}_bgimg.{ext}`, the original image is removed from the media root so it cannot be indexed twice, and the scan returns the generated `{boardID}.board` item. Existing image display metadata, visible modified time, like count, and comment file are migrated to the new board media ID when present.
