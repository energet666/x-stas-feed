...
- Expanded drawing boards have a read-only history mode entered from the drawing toolbar. In history mode the drawing toolbar is replaced by a bottom range control plus exit button; the range value renders only the first `N` strokes from the board history, while drawing input, brush cursor, active stroke previews, and line previews are disabled.
