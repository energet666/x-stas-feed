...
- `DESIGN.md` documents the in-house design system. New shared styling should prefer semantic `@theme` tokens such as `--color-text-*`, `--color-action-*`, `--color-border-*`, purpose-specific radii, and canonical `ui-*` primitives. Historical `glass-*`, `card-overlay*`, and old color variable names remain compatibility aliases.
